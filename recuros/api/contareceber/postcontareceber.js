...
async function buscarDados(apiUrl) {
    try {
        ...
        console.log(
            `🔄 Resposta da API (${apiUrl}): ${response.status} - ${response.statusText}`
        );
...
function preencherSelect(selectElement, data, campoId, campoNome) {
    console.log(
        `🎯 Preenchendo select #${selectElement.id} com`,
        data.length,
        "itens."
    );
    ...
    if (data.length === 0) {
        console.warn(
            `⚠️ Nenhum dado encontrado para preencher #${selectElement.id}`
        );
    }
...
async function carregarClientes() {
    ...
    if (!selectCliente && !editSelectCliente) {
        console.error(
            "❌ Nenhum select encontrado (IDs: clienteId ou edit_clienteId)"
        );
        return;
    }
        ...
        try {
            ...
            if (!clientes?.length) {
                ...
                if (editSelectCliente)
                    editSelectCliente.innerHTML = noDataOption;
                return;
            }
            ...
            const clientesFiltrados = clientes.filter(
                (cliente) =>
                    ...
                    (cliente.state === 0 || cliente.state === "0")
            );
            ...
            if (clientesFiltrados.length === 0) {
                ...
                if (editSelectCliente)
                    editSelectCliente.innerHTML = noValidOption;
                return;
            }
            ...
            if (selectCliente)
                preencherSelect(selectCliente, clientesFiltrados, "id", "nome");
            if (editSelectCliente)
                preencherSelect(
                    editSelectCliente,
                    clientesFiltrados,
    ...
    if (selectCliente) {
        selectCliente.addEventListener("focus", carregarListaClientes, {
            once: true,
        });
    }
    if (editSelectCliente) {
        editSelectCliente.addEventListener("focus", carregarListaClientes, {
            once: true,
        });
    }
}
...
async function carregarProjetos() {
    ...
    if (!selectProjeto && !editSelectProjeto) {
        console.error(
            "❌ Nenhum elemento select encontrado (projectoId ou edit_projectoId)."
        );
        return;
    }
    ...
    const defaultOption =
        "<option value='' hidden disabled selected>Selecione um projeto</option>";
    ...
    const handleFocus = async () => {
        ...
        try {
            ...
            if (projetos?.length > 0) {
                ...
                if (projetosFiltrados.length > 0) {
                    ...
                    const noProjectsOption =
                        "<option disabled selected>Nenhum projeto disponível</option>";
                    if (selectProjeto)
                        selectProjeto.innerHTML = noProjectsOption;
                    if (editSelectProjeto)
                        editSelectProjeto.innerHTML = noProjectsOption;
                }
            } else {
        ...
        if (selectProjeto)
            selectProjeto.removeEventListener("focus", handleFocus);
        if (editSelectProjeto)
            editSelectProjeto.removeEventListener("focus", handleFocus);
    };

    // Adiciona o evento de focus a ambos os selects (se existirem)
    if (selectProjeto)
        selectProjeto.addEventListener("focus", handleFocus, { once: true });
    if (editSelectProjeto)
        editSelectProjeto.addEventListener("focus", handleFocus, {
            once: true,
        });
}
...
async function carregarBancos() {
    ...
    if (!selectBanco && !editSelectBanco) {
        console.error(
            "❌ Nenhum elemento select encontrado (recebidoPeloBancoId ou edit_recebidoPeloBancoId)"
        );
        return;
    }
    ...
    const defaultOption =
        "<option value='' hidden disabled selected>Selecione um banco</option>";
    ...
    const carregarListaBancos = async () => {
        ...
        try {
            ...
            if (!bancos?.length) {
                ...
                const noDataOption =
                    "<option disabled selected>Nenhum banco disponível</option>";
            ...
            if (bancosFiltrados.length === 0) {
                ...
                const noValidOption =
                    "<option disabled selected>Nenhum banco válido</option>";
    ...
    // Adiciona o evento de focus (carrega apenas uma vez)
    if (selectBanco) {
        selectBanco.addEventListener("focus", carregarListaBancos, {
            once: true,
        });
    }
    if (editSelectBanco) {
        editSelectBanco.addEventListener("focus", carregarListaBancos, {
            once: true,
        });
    }
}
